...
public class AlipayConfigImpl implements IAlipayConfig {
	...
	private String processParams(Map<String, String> param, String type) {
		...
		if ("1".equals(type)) {// config.set商户信息配置请求参数
			...
			params.put("equipment_name", param.get("equipment_name"));// isv名称
																		// 杭州立方
																		// 非必填
			params.put("out_parking_id", param.get("out_parking_id"));// 全局唯一的停车场编号(由设备商系统提供)
																		// 10000110001
			params.put("parking_address", param.get("parking_address"));// 停车场地址
			params.put("longitude", param.get("longitude"));// 经度，最长15位字符（包括小数点），注：高德坐标系
															// 114.266418 非必填
			params.put("latitude", param.get("latitude"));// 纬度，最长15位字符（包括小数点），注：高德坐标系
															// 30.548828 非必填
			params.put("parking_start_time", param.get("parking_start_time"));// 营业时间
																				// 07:00:00
			params.put("parking_end_time", param.get("parking_end_time"));// 结束时间
																			// 03:07:50
			params.put("parking_number", param.get("parking_number"));// 停车位数目
																		// 100
			params.put("parking_lot_type", param.get("parking_lot_type"));// 停车场类型，1为小区停车场、2为商圈停车场、3为路面停车场、4为园区停车场、5为写字楼停车场、6为私人停车场
			params.put("parking_type", param.get("parking_type"));// 停车场类型(1为地面，2为地
																	// 下，3为路边)（多个类型，中间用,隔开
			...
			params.put("parking_fee_description", param.get("parking_fee_description"));// 收费说明
																						// 小车一小时10元
																						// 非必填
